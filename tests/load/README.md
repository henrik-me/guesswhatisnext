...
Artillery is a dev dependency that requires **Node.js >= 22.13.0 to run**.
`npm install` installs it by default on any Node version (it's a regular
devDependency), but on Node < 22.13 the install will emit an `engines`
warning and `npx artillery …` will fail at run time.

```bash
npm install          # installs artillery on any Node version (warning only on Node < 22.13)
```

> **Note:** To actually run load tests you need Node >= 22.13. Production
> Docker images use `npm ci --omit=dev` and skip artillery (and the rest
> of the dev tree) entirely. The rest of the project works with Node.js v20+.
