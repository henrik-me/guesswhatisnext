...
**Date:** 2026-05-10 — three latent bugs (CS79, CS80, CS81) surfaced and were resolved across five prod-deploy attempts in ~6 hours, building on the earlier CS73 wake-db fix that landed 2026-05-03 (PR [#330](https://github.com/henrik-me/guesswhatisnext/pull/330) — triggering failure run [25263941898](https://github.com/henrik-me/guesswhatisnext/actions/runs/25263941898) on the CS52-11 deploy ceremony). Closed via [CS81 PR #333](https://github.com/henrik-me/guesswhatisnext/pull/333) and prod-deploy run [25630828772](https://github.com/henrik-me/guesswhatisnext/actions/runs/25630828772) (image `ffccb0f` → revision `gwn-production--0000025`).

**The chain.** What started as a routine prod deploy on the morning of 2026-05-10 uncovered three further latent bugs in succession. Each fix exposed the next:

| # | Run | Image | Failure mode | Resolved by |
|---|-----|-------|--------------|-------------|
| 0 | [25263941898](https://github.com/henrik-me/guesswhatisnext/actions/runs/25263941898) | (older) | Migration step failed connecting to paused Azure SQL serverless (5s connect timeout vs ~30–60s resume window) | [CS73](project/clickstops/done/done_cs73_prod-deploy-cold-db-handling.md) PR [#330](https://github.com/henrik-me/guesswhatisnext/pull/330) — added `scripts/wake-db.js` step before migrations. Wake step worked correctly on every subsequent run in this chain |
| 1 | [25617860563](https://github.com/henrik-me/guesswhatisnext/actions/runs/25617860563) | `fa74aec` | Wake + migrations succeeded; CS41-1 smoke `/api/features` returned 503 in 1–2ms × 18 retries = 90s budget exhausted → auto-rollback. The endpoint was gated by the cold-init request gate but the smoke probe sent no `X-User-Activity: 1` header, so init never fired | [CS79](project/clickstops/done/done_cs79_api-features-cold-init-gate.md) — smoke sends the header |
| 2 | [25619591048](https://github.com/henrik-me/guesswhatisnext/actions/runs/25619591048) | (next) | CS79 fix worked (features ok, score POST ok). CS41-1 smoke step (e) `GET /api/scores/me` returned 503: `AVG(score)` overflowed 32-bit `INT` accumulator after enough accumulated rows. SQLite never reproduced this (64-bit ints natively) | [CS80](project/clickstops/done/done_cs80_scores-avg-int-overflow.md) — `CAST(score AS BIGINT)` before `AVG`/`SUM` |
| 3 | [25621131079](https://github.com/henrik-me/guesswhatisnext/actions/runs/25621131079) | (next) | CS80 cast landed in NEW image, but CS41-12 OLD-revision smoke against migrated DB still failed: the OLD revision had the un-cast aggregate, and accumulated test-bot rows still triggered the overflow. Chicken-and-egg: the gate verifies OLD compatibility but OLD has the bug. **Halted before traffic shift** — not a rollback | [CS81](project/clickstops/done/done_cs81_test-data-hygiene.md) — drain accumulated `gwn-smoke-bot` rows + add per-test cleanup |
| 4 | [25630828772](https://github.com/henrik-me/guesswhatisnext/actions/runs/25630828772) | `ffccb0f` | ✅ green end-to-end — wake step 1s (DB warm), CS41-12 OLD smoke ✅, CS41-1 NEW smoke ✅, all four CSes (CS73, CS79, CS80, CS81) closed by this single deploy |
...
- CS41-1 auto-rollback fired correctly on smoke failure (rows #1, #2) → traffic shifted back to the previous green revision before users could see anything.
- CS41-12 OLD-revision smoke gate halted row #3 BEFORE traffic shift → prod stayed on the previous green revision, no rollback needed.
...
| express-rate-limit | Rate limiting | v8.5.1 — auth endpoints, telemetry, submissions (CS78 bumped 8.3.1 → 8.5.1; the bump pulls `ip-address@10.2.0` transitively, which matches `express-rate-limit@8.5.1`'s natural floor — the CVE itself is patched starting at `ip-address` 10.1.1) |
