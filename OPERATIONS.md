...
### CS41-12 chicken-and-egg pattern

When a deploy includes an app-code fix (not a migration) that the smoke probe exercises, AND the OLD revision lacks that fix, AND the smoke surfaces the bug: CS41-12 will halt the deploy before traffic shift. This is intentional safety — but it creates a chicken-and-egg where the deploy gate verifies OLD compatibility but OLD has the bug we're fixing.
