...
| CS53-20 | **CD-side cold-start *assertion* smoke (validates CS53-10 simulator end-to-end)**<br>WT: `C:\src\gwn-worktrees\wt-cs53-20`<br>B:&nbsp; `yoga-gwn/cs53-20-cold-start-assert-smoke` | copilot_review | yoga-gwn | 2026-04-27T04:43Z | — |
|  | _**PR [#305](https://github.com/henrik-me/guesswhatisnext/pull/305) OPEN** (R1 fix `5c260d9` pushed). New `cold-start-assert-smoke-test` job in staging-deploy.yml — same image as smoke-test but with `GWN_ENABLE_DB_CONNECT_SIMULATORS=1` + `GWN_SIMULATE_COLD_START_FAILS=3`. Probes `/api/scores/leaderboard?variant=freeplay` with `X-User-Activity:1` and asserts ≥1× 503+Retry-After then 200 within 90s; second step greps container stdout for `mode=cold-start-fails` Pino warns (exactly 3 expected; FAILS if any `mode=unarmed:*` matches per GPT-5.4 R1 finding — catches arming-gate regression that would otherwise silently render assertion no-op). `update-staging-branch.needs` includes the new job so a regression blocks deploy. First end-to-end CD validation of the CS53-10 simulator (PR #301)._ |  |  |  |  |
