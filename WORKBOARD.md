...
| CS52-4 | CS52 | claimed | yoga-gwn-c5 | C:\src\gwn-worktrees\wt-cs52-4 | yoga-gwn-c5/cs52-4-impl | -- | 2026-04-26T18:30Z | Sub-agent dispatched (background, claude-opus-4.7). Building: client mode picker (Ranked entry points on Free Play + Daily screens; Local renamed to Practice in UI), connectivity state machine wiring (per-state banners; disable Ranked in non-ok states), single combined claim prompt on first sign-in, mid-Ranked-disconnect hard-fail overlay. Depends on CS52-3 + CS52-5 (both merged). | -- |
