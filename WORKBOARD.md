...
> **Last updated:** 2026-04-29T04:08Z
...
| OPS-checklist-hardening | **Tighten Quick Reference Checklist + bump local-review floor to GPT 5.5**<br>WT: `C:\src\gwn-worktrees\wt-1`<br>B:&nbsp; `omni-gwn/ops-checklist-hardening` | claimed | omni-gwn | 2026-04-29T04:08Z | — |
|  | _Three-part docs edit: (1) add explicit `hostname` step at session start so agent ID is derived from machine, not pattern-matched from cwd path (closes gap where omni-gwn was misidentified as yoga-gwn this session — rule already exists in OPERATIONS.md startup step 5 + TRACKING.md § Agent Identification but is missing from the only checklist re-read every pull); (2) make "re-read INSTRUCTIONS.md fully if it changed" verifiable — require viewing the entire file (multiple `view_range` calls if >50KB) and stating "INSTRUCTIONS.md re-read complete @ &lt;SHA&gt;" before other work; (3) bump local-review floor `gpt-5.4` → `gpt-5.5` (Opus stays for regular work) across INSTRUCTIONS.md, REVIEWS.md, OPERATIONS.md, README.md; generalize REVIEWS.md heading to `## Local Review Loop` (drop the version) and update anchor cross-refs so future model bumps don't churn anchors. Leave historical references in done/active CS files alone. Docs-only PR; `check:docs:strict` gate; local **GPT-5.5** review loop; skip Copilot per docs-only policy._ |  |  |  |  |
