...
| CS64 | **Planning Conventions Deps And Parallelism**<br>WT: `C:\src\gwn-worktrees\wt-1`<br>B:&nbsp; `omni-gwn/cs64-planning-conventions` | ready_to_merge | omni-gwn | 2026-04-29T17:08Z | Awaiting user `--admin` squash-merge approval (docs-only PR, all CI checks green, GPT-5.5 local review clean R1; no APPROVED review path because Copilot is skipped per docs-only policy — same situation as PR #310 earlier today). |
|  | _PR [#312](https://github.com/henrik-me/guesswhatisnext/pull/312). Once merged, CS65/66/67 unblock and fan out in parallel. Three sub-agent dispatches will follow immediately on three worktrees (wt-2, wt-3, wt-4)._ |  |  |  |  |
| CS65 | **Plan File Schema Linter Rules (CS62 follow-up)**<br>WT: _(pending — dispatched after CS64 merges)_<br>B:&nbsp; _(pending)_ | blocked | omni-gwn | 2026-04-29T16:52Z | Awaiting CS64 merge (linter rules enforce CS64's frontmatter convention). |
|  | _Will add 5 warn-only linter rules: `plan-has-depends-on`, `plan-has-parallel-safe-with`, `plan-has-status-line`, `plan-has-required-sections`, `plan-task-id-format`. CS65-2 also closes CS62's open promise (flips its two existing warn-only rules to error after baseline cleanup)._ |  |  |  |  |
| CS66 | **PR Body And Commit Trailer Gates**<br>WT: _(pending — dispatched after CS64 merges)_<br>B:&nbsp; _(pending)_ | blocked | omni-gwn | 2026-04-29T16:52Z | Awaiting CS64 merge (gate's PR-body schema docs reference the new convention). |
|  | _Will add `scripts/check-pr-body.js` and `scripts/check-commit-trailers.js` as CI gates on `pull_request: [opened, synchronize, edited]`. Soaks non-required, then promoted to required status checks._ |  |  |  |  |
| CS67 | **Canonical Sub Agent Checklist And Conventions Doc (INSTRUCTIONS split)**<br>WT: _(pending — dispatched after CS64 merges)_<br>B:&nbsp; _(pending)_ | blocked | omni-gwn | 2026-04-29T16:52Z | Awaiting CS64 merge (high INSTRUCTIONS.md/OPERATIONS.md collision with CS64; serialize). |
|  | _Phase 1: extract OPERATIONS.md § Sub-Agent Checklist into `docs/sub-agent-checklist.md` + drift-check linter rule. Phase 2: extract code/test/architecture sections from INSTRUCTIONS.md into `CONVENTIONS.md`; update repo-wide cross-refs._ |  |  |  |  |
