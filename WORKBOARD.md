...
| CS65 | **Plan File Schema Linter Rules (CS62 follow-up)**<br>WT: `C:\src\gwn-worktrees\wt-2`<br>B:&nbsp; `omni-gwn/cs65-plan-file-schema-linter` | claimed | omni-gwn | 2026-04-29T17:14Z | — |
|  | _CS64 merged (`bc6065c` via PR #312). Adds 5 warn-only linter rules to `scripts/check-docs-consistency.js`: `plan-has-depends-on`, `plan-has-parallel-safe-with`, `plan-has-status-line`, `plan-has-required-sections`, `plan-task-id-format`. Code change → local + Copilot review path. CS65-2 (flip-to-error) is a follow-up after soak window._ |  |  |  |  |
| CS66 | **PR Body And Commit Trailer Gates**<br>WT: `C:\src\gwn-worktrees\wt-3`<br>B:&nbsp; `omni-gwn/cs66-pr-body-and-trailer-gates` | claimed | omni-gwn | 2026-04-29T17:14Z | — |
|  | _CS64 merged. Adds `scripts/check-pr-body.js` + `scripts/check-commit-trailers.js` + `.github/workflows/pr-checks.yml` job. Code/CI change → local + Copilot review path. Soaks as non-required first, promoted to required in CS66-3 follow-up._ |  |  |  |  |
| CS67 | **Canonical Sub Agent Checklist And Conventions Doc (INSTRUCTIONS split)**<br>WT: `C:\src\gwn-worktrees\wt-4`<br>B:&nbsp; `omni-gwn/cs67-canonical-checklist-and-conventions` | claimed | omni-gwn | 2026-04-29T17:14Z | — |
|  | _CS64 merged. Phase 1: extract OPERATIONS.md § Sub-Agent Checklist into `docs/sub-agent-checklist.md` + add `sub-agent-checklist-canonical` drift rule (small code change). Phase 2: extract `CONVENTIONS.md` from INSTRUCTIONS.md (pure docs). Sub-agent decides whether to split into 2 PRs based on size; mixed code+docs → local + Copilot review path._ |  |  |  |  |
