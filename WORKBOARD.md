...
> **Last updated:** 2026-05-09T23:55Z
...
| CS73 | **Prod-Deploy Cold DB Handling**<br>WT: `C:\src\guesswhatisnext_cs73`<br>B:&nbsp; `cs73-wake-db` | implementing | yoga-gwn | 2026-05-09T23:55Z | — |
|  | _Implementing CS73-1 (`scripts/wake-db.js` with retry/backoff, bypasses `server/db/mssql-adapter.js` to preserve CS53 5s timeout) + CS73-2 (workflow step in `prod-deploy.yml`) + CS73-3 (unit tests at `tests/wake-db-script.test.js`) + CS73-4 (LEARNINGS.md update). Sub-agent dispatched 2026-05-09T23:55Z. CS73 closure waits for natural cold-pause prod deploy validation post-merge (no manual pause possible — `az sql db pause` doesn't support GP_S_Gen5 serverless). Plan: [active_cs73_prod-deploy-cold-db-handling.md](project/clickstops/active/active_cs73_prod-deploy-cold-db-handling.md). Unblocks CS75._ |  |  |  |  |
