...
| CS65-2 | **Plan-File Schema Linter Rules — Baseline Cleanup + Flip to Error**<br>WT: `C:\src\gwn-worktrees\wt-2`<br>B:&nbsp; `omni-gwn/cs65-2-flip-to-error` | claimed | omni-gwn | 2026-04-29T20:55Z | — |
|  | _Closes the CS62 + CS65 open promise. Per user direction 2026-04-29T20:54Z: clean up baseline warnings on `project/clickstops/{planned,active}/` then flip warn-only rules to error. (1) Fix 9 `clickstop-h1-matches-filename` warnings on planned/active by aligning H1 to filename slug (preserves stable filenames; WORKBOARD CS52-11 + CS60 titles pre-updated by orchestrator to match new H1s). (2) Scope CS62's `clickstop-h1-matches-filename` rule from `{planned,active,done}` to `{planned,active}` so done/ historical files stay out of scope (no revisionist updates). (3) Flip 7 warn-only rules to error in `--strict` mode: CS62's `clickstop-h1-matches-filename` + `workboard-title-matches-h1`; CS65's `plan-has-depends-on` + `plan-has-parallel-safe-with` + `plan-has-status-line` + `plan-has-required-sections` + `plan-task-id-format`. CS67-1's `sub-agent-checklist-canonical` stays warn-only for now (different scope, separate decision). Code PR (script change) → local + Copilot review path._ |  |  |  |  |
