...
| CS52-7e | CS52 | copilot_review | yoga-gwn-c5 | C:\src\gwn-worktrees\wt-cs52-7e | yoga-gwn-c5/cs52-7e-impl | #267 | 2026-04-26T18:51Z | PR #267 open. pending_writes durable queue + request-bound drain (3 variants: /finish, /sync, INTERNAL multiplayer-match-completion). No-timer property enforced by test. 687/687 vitest pass; lint clean; container:validate PASSED; local code-review (gpt-5.4) pass with 5/6 findings addressed (validator parity at replay, broader transient-error tagging, drain-again flag, parent-dir fsync, dead-letter on corrupt JSON). 1 deferred: GWN_DATA_DIR durability under Container Apps (same /tmp limitation as DB itself). Awaiting Copilot review. | -- |> **Note:** Clickstop files live under lifecycle subdirectories: `project/clickstops/planned/` (queued), `project/clickstops/active/` (in flight), `project/clickstops/done/` (completed). See the task tables inside those files for task-level status. Completion history is recoverable via `git log --diff-filter=A -- project/clickstops/done/`.
