...
| CS41-0 | Reserve `gwn-smoke-*` prefix + filter from public surfaces + setup script | implementing | yoga-gwn-c2 (sub-agent) | C:\src\guesswhatisnext_copilot2 | yoga-gwn-c2/cs41-0-smoke-user-reservation | -- | 2026-04-26T03:35Z | Sub-agent implementing in worktree. Will land: (1) registration rejects `gwn-smoke-*` prefix, (2) leaderboard + community-puzzle endpoints filter the prefix, (3) idempotent `scripts/setup-smoke-user.js` for operator one-time creation. Tests + container-validate + telemetry section to follow. | -- |
