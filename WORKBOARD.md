...
<!-- check:ignore no-orphan-active-work -->
| CS67-2 | **CS67 Phase 2 — Extract CONVENTIONS.md from INSTRUCTIONS.md**<br>WT: `C:\src\gwn-worktrees\wt-4`<br>B:&nbsp; `omni-gwn/cs67-2-conventions-md` | claimed | omni-gwn | 2026-04-29T20:22Z | — |
|  | _CS67 Phase 1 (canonical sub-agent checklist + drift rule) merged via PR #313 / commit `9472757`. OPS-clarify-copilot-review-merge-gate merged via PR #316 / commit `4f5a82e`. Phase 2 unblocked. Heavy INSTRUCTIONS.md restructure: extract architecture/coding/testing/logging/telemetry/git-workflow/db/performance sections into a new `CONVENTIONS.md` at repo root; trim INSTRUCTIONS.md to Quick Reference + orchestrator-only items + pointers; sweep repo-wide `INSTRUCTIONS.md#anchor` cross-refs to point at `CONVENTIONS.md#anchor`. Pure docs PR (no code change) → local GPT-5.5 review only, no Copilot per docs-only policy._ |  |  |  |  |
