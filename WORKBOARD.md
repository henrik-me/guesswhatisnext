...
| OPS-checklist-hardening | **Tighten Quick Reference Checklist — hostname-based agent ID derivation + checkable INSTRUCTIONS.md re-read after pull**<br>WT: `C:\src\gwn-worktrees\wt-1`<br>B:&nbsp; `omni-gwn/ops-checklist-hardening` | claimed | omni-gwn | 2026-04-29T04:08Z | — |
|  | _Two-line edit to INSTRUCTIONS.md Quick Reference Checklist: (1) add explicit `hostname` step at session start so agent ID is derived from machine, not pattern-matched from cwd path (closes gap where omni-gwn was misidentified as yoga-gwn this session — the rule already exists in OPERATIONS.md startup step 5 + TRACKING.md § Agent Identification but is missing from the only checklist re-read every pull); (2) make "re-read INSTRUCTIONS.md fully if it changed" verifiable — require the agent to view the entire file (multiple `view_range` calls if >50KB) and state "INSTRUCTIONS.md re-read complete @ <SHA>" before doing other work. Docs-only PR; `check:docs:strict` gate; local GPT-5.4 review loop; skip Copilot per docs-only policy._ |  |  |  |  |
