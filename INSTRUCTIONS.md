...
- **Lint-before-push is mandatory on every direct-to-main push** (WORKBOARD edits and clickstop plan files): run `npm run check:docs:strict` locally and only push when the linter is clean (or only the pre-existing CS60 WORKBOARD finding remains). The orchestrator's direct-push admin-bypasses ALL server-side required status checks (branch protection has no per-rule granularity), so the linter is the only gate. [CS77](project/clickstops/active/active_cs77_pre-push-docs-lint-hook.md) ships a husky `pre-push` hook that automates this, but it activates per-clone via `npm install`; until each existing checkout has run `npm install` since CS77 landed (tracked in CS77-5), the manual `check:docs:strict` reflex remains the safety net. The CS77-2c warning fires automatically when the hook is missing. See [§ WORKBOARD.md — Live Coordination in TRACKING.md](TRACKING.md#workboardmd--live-coordination) for the full rationale.
...
- **Plan files (`project/clickstops/{planned,active,done}/*.md`) require lint + GPT-5.5 review BEFORE the direct-to-main push, not after.** "Commit clickstop plan file to main BEFORE starting implementation work" is about plan-vs-implementation sequencing, not a license to skip review on the plan itself. Sequence: write → `npm run check:docs:strict` → fix → dispatch `code-review` sub-agent (`model=gpt-5.5` floor) → apply fixes → re-lint → commit + push. Hit on CS75 push `359fa00` (3 lint errors shipped) and CS78 plan (artillery=`optionalDependencies` factual error caught by review).
- **Always deploy main → staging → prod, even when the changed code path doesn't apply functionally to staging.** The clean staging deploy itself is a no-regression test (`npm ci`, container build, boot, Ephemeral Smoke). "The change doesn't apply to staging" (e.g. Azure SQL-only changes) is NOT a reason to skip staging — see [§ Production deploys](#production-deploys--approval-gate-is-on-the-user) for the full sequence and rationale.
...
- **`git mv X Y` + content edits to that same file in one `git add` may commit only the rename.** Always `git status --short` after the commit; if the commit summary is `1 file changed, 0 insertions, 0 deletions`, your content edits did NOT make it in — follow up with a second commit before pushing. Hit twice in the CS77/CS78 lifecycle close-out sequence.
...
- **PR body schema** — every PR body must contain `## Local Review`, `## Container Validation`, and `## Telemetry Validation` sections in the formats defined in [REVIEWS.md § Local Review Loop](REVIEWS.md#local-review-loop), [OPERATIONS.md § Cold-start container validation](OPERATIONS.md#cold-start-container-validation), and [CONVENTIONS.md § 4a Telemetry & Observability](CONVENTIONS.md#4a-telemetry--observability-mandatory-for-all-new-work) respectively. Operational sections may use `not applicable (<category>)` for docs-only / tooling-only / CI-config-only / docs/CI-only PRs and supported `+` combinations (with optional clarification). The `PR body process gate` and `Commit trailer gate` CI checks run on every PR and are required by process; after CS66-3 merges, the orchestrator promotes them to required branch-protection status checks. Scan their failure output for `See: <doc>` links.
...
**Standard deploy sequence — main → staging → prod.** After any PR merges to main, the canonical sequence is:

1. `gh workflow run staging-deploy.yml --ref main` (no inputs).
2. Surface the staging approval link prominently AND dispatch a background watcher per § watcher rules below.
3. Wait for staging to complete green (5 jobs: Build & Push, Cold-start assertion smoke, Ephemeral Smoke, Fast-Forward release/staging, Deploy to Azure Staging).
4. `gh workflow run prod-deploy.yml --ref main -f image-tag=<short-sha-from-staging-build> -f confirm=production`.
5. Surface the prod approval link prominently AND dispatch a background watcher.

This sequence applies even when the merged change doesn't functionally exercise staging (e.g. Azure SQL-only changes against staging's container-local SQLite). The clean staging deploy itself is a valid no-regression test (npm ci, container build, boot, Ephemeral Smoke). Skipping staging on grounds of "the change doesn't apply there" is a process violation.
...
Note that both Azure `gwn-staging` and `gwn-production` are being moved to `minReplicas: 0` (scale-to-zero; live staging state tracks [CS58-1/CS58-2](project/clickstops/done/done_cs58_scale-staging-to-zero.md), live prod state tracks [CS75](project/clickstops/active/active_cs75_scale-prod-to-zero.md)). Neither environment is a pre-prod release gate — the enforced gate is the in-CI Ephemeral Smoke Test job in [`.github/workflows/staging-deploy.yml`](.github/workflows/staging-deploy.yml) plus local `npm run container:validate` cycles. See [§ Waking staging for ad-hoc validation in OPERATIONS.md](OPERATIONS.md#waking-staging-for-ad-hoc-validation) and [§ Waking production for ad-hoc validation in OPERATIONS.md](OPERATIONS.md#waking-production-for-ad-hoc-validation) for the operator probe procedures.
