...
- **Boot-quiet rule (CS53-23 / CS53-19).** No boot, focus, refresh, bfcache-restore, or service-worker-lifecycle code path on the client may fire a request that touches the DB. The mechanism is the **`X-User-Activity: 1` request header**: clients set it on requests originating from a real user gesture; servers MUST NOT issue any DB query in response to a request that lacks the header (or carries any other value). On cache miss without the header, the server returns the cached value if any, else the empty default for that endpoint (`{ unread_count: 0 }` for `/api/notifications/count`, `204 No Content` where no body shape is meaningful, etc.). The header is the boot-quiet contract that lets the server act as the guard against untrusted or stale clients without heuristics. Cross-link: `project/clickstops/active/active_cs53_prod-cold-start-retry-investigation.md` row CS53-23 (contract foundation, fully wired today only on `/api/notifications/count`) and CS53-19 (apply contract to every other boot/focus endpoint).
