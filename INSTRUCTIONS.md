...
- **PR body schema** — every PR body must contain `## Local Review`, `## Container Validation`, and `## Telemetry Validation` sections in the formats defined in [REVIEWS.md § Local Review Loop](REVIEWS.md#local-review-loop), [OPERATIONS.md § Cold-start container validation](OPERATIONS.md#cold-start-container-validation), and [CONVENTIONS.md § 4a Telemetry & Observability](CONVENTIONS.md#4a-telemetry--observability-mandatory-for-all-new-work) respectively. Operational sections may use `not applicable (<category>)` for docs-only / tooling-only / CI-config-only PRs (with optional clarification). The `PR body process gate` and `Commit trailer gate` CI checks are required — scan their failure output for `See: <doc>` links.
