...
- **Long-running PR + churning main → use `gh pr merge --squash --admin` once CI is green and review approved.** Branch protection's up-to-date requirement combined with active main churn from parallel agents creates infinite-rebase loops. `gh pr merge --auto` is disabled in this repo. Heuristic: if a PR has been ready-to-merge ≥30 min and main has moved ≥3 times since the last successful CI, escalate to `--admin` rather than another rebase. Document the merge in the PR comment thread when using `--admin`.
