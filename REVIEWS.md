...
## Local Review Loop (Latest GPT Model)

Before requesting Copilot PR review, sub-agents **must** run a local review loop using the `code-review` agent with the latest available GPT model (`gpt-5.4` at time of writing — always prefer the newest GPT release available). This catches issues in ~60 seconds vs the 2-10 minute Copilot polling cycle.

**Local review procedure:**
1. After pushing changes and creating the PR, launch a local review:
   ```
   task agent_type=code-review model=gpt-5.4:  # use latest available GPT model
