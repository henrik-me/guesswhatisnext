...
- On Node >= 22.13, `npm install` installs the `artillery` dev dependency and may emit OpenTelemetry peer-dependency warnings. Those warnings come from load-testing dependencies that still pull older OTel metrics/exporter packages, while the application's runtime telemetry path uses newer OTel packages.
