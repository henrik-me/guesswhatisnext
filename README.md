...
- `npm install` installs the `artillery` dev dependency on any Node version and may emit OpenTelemetry peer-dependency warnings during install. Those warnings come from load-testing dependencies that still pull older OTel metrics/exporter packages, while the application's runtime telemetry path uses newer OTel packages. Node >= 22.13 is required to *run* artillery, but the install-time warnings can appear on any Node version that installs dev deps.
- This is currently treated as non-blocking install noise for the load-testing tooling. It does not affect the validated runtime telemetry path, and no dependency changes are planned right now. If you do not need load testing, `npm ci --omit=dev` avoids the warning by skipping dev dependencies such as `artillery` (this is also what the production Docker image does).
...
**Current workflow:** Push branch → create PR → local review loop (GPT 5.5) → Copilot review (code/config PRs only; docs-only PRs skip) → squash-merge to main.
