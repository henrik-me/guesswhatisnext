...
- **Title cell, lines 2-3:** `WT: ` `worktree-path` ` (line 2) and `B:&nbsp; ` `branch-name` ` (line 3), separated by `<br>`. The `&nbsp;` keeps the `B:` and worktree label visually aligned in rendered markdown.
...
- **Non-CS work** (ad-hoc requests, deployments, investigations not tied to a clickstop): use a placeholder `CS-Task ID` such as `OPS-DEPLOY-2026-04-27` so the cell is non-empty (otherwise the parser will treat the status row as a description-continuation row and skip it). The placeholder need not match any CS file; the `workboard-title-matches-h1` rule short-circuits when no parent CS file is found.
