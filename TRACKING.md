...
**Lint-before-push is mandatory** for both direct-to-main paths (WORKBOARD and clickstop plan files). The orchestrator's direct push admin-bypasses **all** required server-side status checks alongside the PR requirement — branch protection has no per-rule granularity that would let `check:docs:strict` keep firing while the PR rule is bypassed (verified via `remote: Bypassed rule violations` log on the CS75 push of `359fa00`, 2026-05-08). [CS77](project/clickstops/active/active_cs77_pre-push-docs-lint-hook.md) provides a husky `pre-push` hook that runs `check:docs:strict` automatically, but it activates per-clone via `npm install`; until each existing checkout has run `npm install` since CS77 landed (tracked in CS77-5), every orchestrator must continue to run `npm run check:docs:strict` locally before each direct-to-main push and only push when the linter is clean (or only the pre-existing CS60 WORKBOARD finding remains). The hook's CS77-2c warning fires automatically on `npm run check:docs:strict` runs in clones where the hook is missing, so the manual reflex is self-correcting on first lint. This applies even to single-line WORKBOARD edits.

The workboard must be updated immediately when:
