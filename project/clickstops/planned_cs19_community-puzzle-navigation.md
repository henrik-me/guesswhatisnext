...
| CS19-2 | Gate sub-page navigation behind feature flag | ⬜ Pending | Ensure the `submitPuzzle` feature flag controls visibility of the Community Puzzles menu item. When flag is off, the menu item and sub-page are completely hidden. |
...
- **Feature flag reuse:** Reuse the existing `submitPuzzle` feature flag — no new flag needed. The flag already has rollout percentage, user targeting, and environment restrictions.
