...
| CS73-3 ✅ Done (PR #330) | Added unit tests for `scripts/wake-db.js` in `tests/wake-db-script.test.js` (matching the existing naming pattern of `tests/migrate-script.test.js`). Mocks `mssql` via the DI seam from CS73-1. Cases cover: success on first attempt; success after retry on transient `40613`; total budget exhausted → exit 1; default + override `connectionTimeout`/`requestTimeout` on both top-level and `options.*`; per-attempt timeout clamped to remaining budget; positive-number validation rejects `0`; pool closed in failure path; close-before-sleep ordering; no-sleep-when-budget-too-tight (Copilot R4); non-retryable errors (numbered) surfaced immediately; ELOGIN / ENOTFOUND / "Login failed" message-pattern fail-fast (Copilot R3); `connectionString` defaults via `??` so explicit empty-string never falls back to env (Copilot R5); `DATABASE_URL` env fallback when no connectionString given; CLI exit codes. | Confirms the wake mechanism is robust without needing a live DB. |
