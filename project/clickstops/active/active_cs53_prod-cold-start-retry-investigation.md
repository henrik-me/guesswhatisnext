...
1. **CS53-14 + CS53-16 — Paired SW PR (catch path 503 propagation + `skipWaiting`/`clients.claim`).** P0 — active correctness bug: the stale SW intercepts requests after a deploy, strips `Retry-After` and renders a synthetic `503 (Offline)` that breaks the auth retry loop CS53-17 just shipped. Higher user-impact frequency than CS53-19 because every deploy creates the failure window until clients re-load.
...
| CS53-8b | Public `/api/db-status` endpoint — **ops/health endpoint only**. Reads in-memory state (`dbInitialized` flag, init-guard `isInFlight()`, `getDbUnavailability` cached last-error). **Does NOT issue any DB query.** **No SPA polling.** SPA learns DB state via responses to real user requests (`UnavailableError` / `RetryableError` already in place from PR #234). | ⬜ Pending — P2 | Scope tightened to comply with Policy 1. Safe to probe externally because it does not touch the DB. **Telemetry obligation (CS41-6 gate):** Pino info on each request — `{ event: 'db-status-probe', dbInitialized, isInFlight, lastError }`; KQL § B.x counts probes per minute as a watchdog. |
...
| CS53-10 | **Test out-of-money (`capacity_exhausted`) state end-to-end + improve cold-start fidelity.** Need a way to reliably reproduce Azure SQL Free Tier `capacity_exhausted` (`ELOGIN` with "paused for the remainder of the month" message) AND surface realistic transient failures during cold-start, so the `UnavailableError` banner path AND the warmup-retry path stay regression-protected without changing prod billing. See **§ CS53-10 design (2026-04-24)** below. | ⬜ Pending — P2, design ready | Approach: add `GWN_SIMULATE_DB_UNAVAILABLE={capacity_exhausted,transient}` and `GWN_SIMULATE_COLD_START_FAILS=N` env vars to mssql-adapter; extend `container:validate` with `--mode=` flag; new tests asserting classifier shape match. **Telemetry obligation (CS41-6 gate):** Pino warn per simulated unavailable response — `{ gate: 'simulated-unavailable', mode, route }`; KQL § B.x confirms the env var is unset in staging/prod (sim doesn't leak). |
...
| CS53-13.A | **Instrument auth `AUTH_WARMUP_DEADLINE_MS` exhaustion signal.** When the auth retry loop in `public/js/app.js` exhausts its deadline without a 200, fire a structured Pino line server-side via the next-attempted request OR (preferable) via a one-shot client-side telemetry beacon to a server route (e.g. `POST /api/telemetry/auth-deadline-exhausted` with `{ attempts, elapsedMs, lastStatus, action }`). KQL § B.x counts incidents per day per `action` (`login`/`register`). Must run for ≥1 week post-deploy before CS53-13's decision can be informed by data. | ⬜ Pending — P2 (CS53-13 prereq) | Telemetry obligation IS the task. Code touched: `public/js/app.js` (auth retry loop), `server/routes/telemetry.js` (probably the right home for the new beacon endpoint, alongside existing client error reporting). PR-CI § 4a Telemetry Validation section auto-satisfied. |
| CS53-14 | **SW catch path: distinguish offline vs server error.** `public/sw.js` currently synthesises a `503 (Offline)` response for any failed inner fetch — including warmup 503s from the server's own gate. This corrupts the auth retry loop's view of the response (the SW response has no `Retry-After`, breaking `parseRetryAfterMs`). Solution: SW should propagate the original network error or pass-through the upstream 503 with its headers intact. | ⬜ Pending — P0 (paired with CS53-16) | Manifested during CS53-17 validation when the old SW (`gwn-f224ccd2`) intercepted requests after the new SW shipped but before the user re-loaded. Workaround: unregister SW in DevTools or InPrivate browse. **Telemetry note:** SW runs in browser, NOT subject to CS41-6 server-side gate. PR body must explicitly state "SW change; no new server telemetry required" to satisfy the gate's PR-body grep. Document the SW behavior change in `docs/observability.md` for cross-reference. |
...
| CS53-16 | **SW skipWaiting + clients.claim.** New service-worker versions wait for all clients to close before activating. During CS53-17 iteration this routinely meant the old SW kept intercepting requests for the entire dev session. Adding `self.skipWaiting()` in `install` and `clients.claim()` in `activate` makes new versions take over immediately. | ⬜ Pending — P0 (paired with CS53-14) | Branch created but not committed. Sequence after CS53-14 so the cache-version bump and the catch-path fix ship together. **Telemetry note:** same as CS53-14 — SW runs in browser; no new server telemetry required (declare in PR body). |
...
| **CS53-19.D** | **Apply the contract to server-side handlers.** For every endpoint in 19.A's "DB-touching boot endpoint" column, wire the CS53-23.E/F helper. Targets at minimum: `/api/auth/me`, `/api/features`, `/api/notifications` (list), `/api/notifications/count`, `/api/scores/me`, `/api/achievements`, `/api/matches/history`. Each handler returns the agreed shape (per CS53-23.B) when the header is absent. JWT validation in `/api/auth/me` is the trickiest case — must validate the token (which is in-memory), but must **not** look up the user record in the DB unless the header is present. **Also closes the cold-start init-gate gap (the CS53-23 R6 caveat):** the global `/api/*` request gate at `server/app.js:258-280` calls `runInit()` for header-less requests when `!dbInitialized`, which can touch the DB before an enrolled handler runs. Gate that path on `X-User-Activity: 1` AND update `scripts/container-validate.js` to send the header so the cold-start harness still exercises the init path. | CS53-19.B | If CS53-23 ships the helper as middleware, this may collapse to a one-line `app.use('/api/...', requireUserActivity)` per route group. Prefer that over per-handler edits. |
| **CS53-19.E** | **Boot-quiet regression test.** Add `tests/boot-quiet.spec.mjs` (Playwright E2E) that: (1) launches the local stack, (2) seeds a valid JWT into `localStorage`, (3) navigates to `/`, (4) waits 10s, (5) asserts via a server-side query-counter (or via log scraping of mssql adapter calls) that **zero** DB queries fired during the boot. Then repeat for refresh, refocus, bfcache. **Fail the test if any DB-touching endpoint was called.** Also include system-key bypass coverage (system-key requests always allowed to read DB on cache miss — CS53-23 R4 contract). | CS53-19.D | The query-counter is the contract — without it, the rule will silently regress. Easiest impl: instrument `mssql-adapter._connect()` and `request.query()` to increment a counter, expose via a debug-only endpoint or test-only header. |
...
| **CS53-19.H** | **Per-endpoint telemetry per CS41-6 gate.** For every endpoint enrolled by 19.D, emit a structured Pino line in the same shape as CS53-23's `/api/notifications/count` boot-quiet log (`{ gate: 'boot-quiet', route, cacheOutcome, userActivity, isSystem, userId }`) and add a per-endpoint KQL section in `docs/observability.md` (append at end of § B; accept renumbering on rebase per [LEARNINGS § observability section conflicts](../../../LEARNINGS.md#docsobservabilitymd--bx-section-conflicts-under-parallel-pr-churn-cs53-23)). PR body's `## Telemetry Validation` section asserts each new endpoint emits the line on local validation. | CS53-19.D | Required to satisfy the CS41-6 PR-CI Telemetry Validation gate; without 19.H, the 19.D PR fails CI. May be folded into the same PR as 19.D rather than split. |
