...
| CS53-8 | Implement stuck-state remediation (fixes #4–#8 below): never permanently stop self-init, ~~pool watchdog~~ (cancelled, see CS53-8a), public `/api/db-status`, surface stuck state to users. | ⬜ Pending | Depends on CS53-7 findings. Likely lands as a follow-up PR after the CS53-3 timeout/probe/cap fixes. Scope tightened by the no-DB-wake policy (Policy 1 in INSTRUCTIONS.md). |
...
| CS53-18 | **Raise / unify `WARMUP_CAP_MS` across the SPA.** `public/js/progressive-loader.js:14` still has `WARMUP_CAP_MS = 30000` for every screen *except* auth (auth got its own `AUTH_WARMUP_DEADLINE_MS = 120000` in CS53-17). Container validation routinely shows Azure SQL Free Tier cold-start at ~36–38s, so the 30s cap is **almost certainly still triggering the 3-retry-click symptom on profile/leaderboard/achievements/history in prod today** — i.e. the very bug that triggered CS53. Either raise the global cap to align with auth (120s), or make it adaptive (drive it from the latest server `Retry-After` instead of wall-clock). | ⬜ Pending — P1 | Touches every progressive-loaded surface; needs container validation per Policy 2. The `Retry-After`-driven approach is preferable because it removes the magic number, but the static raise is faster to ship if we want to close the bug immediately. Decide alongside CS53-3 once CS54 logs are available, OR ship the static raise unconditionally now since 120s is already proven safe via CS53-17. |
| CS53-19 | **Boot-time DB-wake audit — apply the X-User-Activity contract to every boot/focus endpoint.** `/api/auth/me` (boot JWT validation), `/api/features`, `/api/notifications/count` and friends all wake the DB on every tab open / refresh / refocus even when no user activity has occurred. HAR evidence (2026-04-25 local-cold-start repro) confirms `/api/features` and `/api/notifications/count` each fire **twice** during a single login boot — both DB-touching, neither user-initiated. CS55-2 owns the *contract* (`X-User-Activity: 1` header + server-side enforcement helper, see CS55-2.G/H). CS53-19 owns *applying* that contract to every endpoint a stale tab can hit. **Hard prerequisite: CS55-2 v2 must ship first** (header name, enforcement helper, and "no header → don't touch DB" response semantics must be defined before this CS can begin Phase C; Phases A and E can run in parallel with CS55-2). | 🔒 Blocked on CS55-2 v2 — P2 | See "## CS53-19 detailed plan" below for sub-tasks A–G. Cross-links: [CS55-2.G/H/J](../planned/planned_cs55_websocket-notifications-and-redesign.md). Acceptance: opening a tab with a valid JWT and immediately closing it causes **zero** DB queries; a CI test enforces this. |
...
| CS53-21 | **Solo-PR merge-gating policy.** PR #244 had to merge with `gh pr merge --admin` because branch protection requires an APPROVED review and the only reviewer (Copilot bot) only ever issues "COMMENTED" reviews. Admin override silently bypasses CI gates as well. Decide: (a) make Copilot bot's "COMMENTED" sufficient when all CI checks pass, (b) configure a "self-merge after N successful CI cycles" rule, (c) accept admin-override as documented policy with a checklist (CI green + local review clean + Copilot has no open findings). | ⬜ Pending — P3 | Could be split out to a new planned clickstop ("CS5x: solo-PR merge policy") since it's process, not CS53 cold-start scope. Keeping it here for now so it's not lost. The ergonomic best answer is probably (a): "approval = at least one COMMENTED review by Copilot AND all CI checks green AND no unresolved review threads". |
...
**Hard prerequisite:** CS55-2 v2 (in `planned_cs55_*.md`) must ship the contract before Phase C can begin:
- **CS55-2.G** — `X-User-Activity` header contract documented in `INSTRUCTIONS.md` (header name, semantics, who sets it client-side, what the server does on absence).
- **CS55-2.H** — server-side enforcement helper (e.g. `requireUserActivity(req, res)` middleware or per-route `if (!req.userActivity) { return res.status(204).end(); }`-style helper) so endpoints don't reimplement the rule.
- **CS55-2.J** — decision on response shape when the header is missing: `204 No Content`, `200 + cached value from process memory`, `200 + null`, or per-endpoint policy. CS53-19 needs this nailed down before it can apply the rule.

Phases A and E can run in parallel with CS55-2; Phases B–D wait.
...
| **CS53-19.B** | **Wait on CS55-2.G/H/J** to define the contract. | CS55-2.G/H/J | Track those rows; close 19.B when the three CS55-2 rows are all marked Done. |
...
| **CS53-19.D** | **Apply the contract to server-side handlers.** For every endpoint in 19.A's "DB-touching boot endpoint" column, wire the CS55-2.H helper. Targets at minimum: `/api/auth/me`, `/api/features`, `/api/notifications` (list), `/api/notifications/count`, `/api/scores/me`, `/api/achievements`, `/api/matches/history`. Each handler returns the agreed shape (per CS55-2.J) when the header is absent. JWT validation in `/api/auth/me` is the trickiest case — must validate the token (which is in-memory), but must **not** look up the user record in the DB unless the header is present. | CS53-19.B | If CS55-2.H is a middleware, this may collapse to a one-line `app.use('/api/...', requireUserActivity)` per route group. Prefer that over per-handler edits. |
...
| **CS53-19.G** | **Document in `INSTRUCTIONS.md`** under § Database & Data: codify "boot-quiet rule" — no boot/focus/SW-lifecycle code path may fire a DB-touching request without a deliberate user gesture marker. Reference the CS55-2 header. | CS55-2.G done | One paragraph. Likely co-shipped with CS55-2.G itself; this row exists so we don't forget if CS55-2 splits the doc update from the code. |
...
CS53-19.A  ─────────────────────────►
                                     ╲
CS55-2.G ──╮                          ╲
CS55-2.H ──┤── CS53-19.B ── CS53-19.C ─┤── CS53-19.E ── done
CS55-2.J ──╯                ╰─────────► CS53-19.D ─┘
                                                    └── CS53-19.F
CS55-2.G ── CS53-19.G (parallel doc update)
```
...
- The `X-User-Activity` header design itself (owned by CS55-2.G).
