...
- **CS53-23.C** — `X-User-Activity` header contract documented in `CONVENTIONS.md` (header name, semantics, who sets it client-side, what the server does on absence).
...
| **CS53-19.G** | ~~**Document in `CONVENTIONS.md`** under § Database & Data: codify "boot-quiet rule" — no boot/focus/SW-lifecycle code path may fire a DB-touching request without a deliberate user gesture marker. Reference the CS53-23 header.~~ | ✅ Done | Boot-quiet rule + cold-start caveat now live in CONVENTIONS § Database & Data (originally landed in INSTRUCTIONS via CS53-23 PR #255) (squash `819e3e1`); wording further refined in PR #282 (squash `472e2a1`). No additional docs work needed. |
...
6. **Docs:** the boot-quiet rule is in `CONVENTIONS.md` and survives the next `npm run check:docs` run.
