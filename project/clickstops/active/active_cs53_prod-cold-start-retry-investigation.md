...
| CS53-22 | **Prod-deploy verify must respect Policy 1 + capacity-exhausted state.** Prod deploy of `cceedac` (run 24935682157) auto-rolled back because `Verify production deployment` polls `/api/health` and requires `db=ok`. With Policy 1 (lazy DB init) the new code returns `db=not_initialized` until something wakes the DB; `/api/health` no longer does that. Worse, when Azure SQL Free Tier is exhausted (the user's current state), `db` will *never* be `ok` even after warming — so the verify step blocks shipping the very fix that handles capacity exhaustion. Two-phase fix needed: (1) actively probe `/api/features` to trigger lazy init OR confirm `503 + unavailable=true + no Retry-After` (intentional unavailability is a successful deploy of the unavailable-handling code); (2) authoritatively check `/api/health` for `status=ok` (and `db=ok` only when phase 1 returned 200). | 🔄 In Progress | Branch `fix/prod-deploy-verify-respect-policy1` opened (PR pending). Edits `.github/workflows/prod-deploy.yml:234-258`. Rubber-duck adopted 3 blocking findings: (a) `/api/features` is not DB-touching by itself, only triggers init — so it's an init trigger not authoritative DB proof; (b) never bless `503 + Retry-After` as success on the last attempt; (c) replace `curl -sf` with status+headers+body capture to actually inspect 503 signals. Verified `sendDbUnavailable()` body shape at `server/app.js:119-126` (`unavailable:true`, `reason`, no `Retry-After`). Once merged + verified on staging, retry prod deploy of `cceedac`. |
