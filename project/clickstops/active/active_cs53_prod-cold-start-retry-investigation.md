...
| CS53-12 | **Fix `ServerResponse` `'finish'` listener leak.** Container logs emit `MaxListenersExceededWarning: Possible EventEmitter memory leak detected. 11 finish listeners added to [ServerResponse]` repeatedly during normal traffic (hot DB, no retries). Suspected sources: OTel HTTP instrumentation, pino-http, and the drain/active-request tracker in `server/app.js` each attach a `'finish'` listener per request. Identify which middleware accumulates without removing, fix, and add a regression guard (assert `res.listenerCount('finish') ≤ N` in an integration test). In CS53 scope because the leak amplifies under cold-start retry storms (each retried request stacks more listeners) and degrades the same UX path CS53 protects. | ✅ Done (PR #242) | Root cause: `@opentelemetry/instrumentation-express` attaches `res.once('finish', ...)` per Express layer (false-positive leak — listeners self-remove). Fix: `res.setMaxListeners(32)` in request-tracking middleware. Regression test: `tests/response-listener-cap.test.js`. |
| CS53-13 | **Retry-button infinite-feeling loop when cold-start consistently exceeds 120s warmup cap.** When the SPA's progressive-loader exhausts its `MAX_WARMUP_BUDGET_MS=120000`, it surfaces the "Taking longer than expected — please try again" + Retry button. Clicking Retry restarts a fresh 120s budget cycle. If the underlying Azure cold-start consistently exceeds 120s, the user sees a perpetual banner→click→wait→banner cycle that *feels* infinite. Design + implement a fix — candidates: (a) Retry button polls `/api/health` (or a new `/api/db-status` from CS53-8b) and only re-issues the real fetch once DB reports ready, with the polling itself surfacing a "still starting…" message; (b) raise `MAX_WARMUP_BUDGET_MS` based on observed Azure p99; (c) keep retrying silently on Retry-click rather than re-showing the banner. Needs UX decision before code. | ⬜ Pending — design | Symptom raised by user 2026-04-24 after observing Retry banner during a sim recreate. Real-prod hit rate depends on actual Azure cold-start tail latency — gather data from CS53-1 logs first to decide if this is hypothetical or real. |
