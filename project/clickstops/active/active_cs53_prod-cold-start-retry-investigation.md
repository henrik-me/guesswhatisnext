...
| CS53-19 | **Boot-time DB-wake audit — single owner.** `/api/auth/me` (boot JWT validation) wakes the DB on every tab open even though no user activity has occurred. Same family of issue as CS55-2 (stale-tab polling waking DB), but neither CS53 nor CS55-2 currently *owns* the fix. CS55-2.E/F (in `planned_cs55_*.md`) sketch the `X-User-Activity` header contract that closes both. Pick one owner (suggest: keep CS55-2 as authoritative for the contract, and let CS53-19 = "audit + apply the contract to every endpoint a stale tab can hit on boot/focus"). | ⬜ Pending — P3 | Cross-links: [CS55-2.E/F](../planned/planned_cs55_websocket-notifications-and-redesign.md). Endpoints to audit at minimum: `/api/auth/me`, `/api/features`, `/api/notifications` (list), `/api/notifications/count`, `/api/scores/me`, `/api/achievements`, `/api/matches/history`. Acceptance: a tab opened with a valid JWT but immediately closed causes zero DB queries. |
