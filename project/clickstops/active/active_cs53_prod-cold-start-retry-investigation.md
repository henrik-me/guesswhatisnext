...
After a code re-read against the current `main`, most rows in the table below that were marked "Pending" or "In Progress" are now superseded by code that landed via PRs #233, #234, #239, #244, #246 and the no-DB-wake policy adoption. Status flips below; **truly remaining work** is:

- **P1** — CS53-14 (SW catch path corrupts upstream 503) **+** CS53-16 (`skipWaiting`/`clients.claim`), shipped together as one paired SW PR.
- **P2** — CS53-8b (`/api/db-status` ops endpoint, in-memory only); CS53-10 (capacity-exhausted regression test, finish PR #240); CS53-19 (boot-quiet enforcement — Phase A HAR can start now, Phases C–G blocked on CS55-2 v2 G/H/J).
- **P3** — CS53-20 (CD-side staging cold-start smoke); CS53-13 (retry-budget UX, defer pending CS54 prod telemetry).
- **Extract from CS53** — CS53-21 (solo-PR merge policy) belongs in its own planned CS, it's process not cold-start.
- **Single biggest external unblock** — CS55-2 v2 (sub-tasks A–L in `planned_cs55_*.md`); PR #241 currently has v1 (5-min TTL) which violates Policy 1.
...
| CS53-21 | **Solo-PR merge-gating policy.** PR #244 had to merge with `gh pr merge --admin` because branch protection requires an APPROVED review and the only reviewer (Copilot bot) only ever issues "COMMENTED" reviews. Admin override silently bypasses CI gates as well. Decide: (a) make Copilot bot's "COMMENTED" sufficient when all CI checks pass, (b) configure a "self-merge after N successful CI cycles" rule, (c) accept admin-override as documented policy with a checklist (CI green + local review clean + Copilot has no open findings). | 🪓 Extract — P3 | **2026-04-25 refresh:** out of CS53 cold-start scope; recommend extracting to a new `planned_csNN_solo-pr-merge-policy.md` (next free CS number) so CS53 can close cleanly when the cold-start work finishes. Recommended landing position is still option (a). |
