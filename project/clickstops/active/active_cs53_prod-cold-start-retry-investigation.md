...
| CS53-11 | **Investigate: cold-start sim env var not producing visible cold-start in browser today.** On 2026-04-24 with `GWN_SIMULATE_COLD_START_MS=45000` propagated to the container env, the app logged `Database self-initialized` (attempt 1) ~2s after boot — the 45s sim never elapsed. Suspected cause: the entrypoint pre-connect probe in `scripts/docker-mssql-entrypoint.sh` consumes the "first connect" slot via a separate Node process, OR app boot eagerly initializes DB despite the CS53-9 lazy-init refactor (need to verify against `server/app.js` boot path). Either way, browser-based validation does not exercise the warmup loader. | ⬜ Pending — diagnose | Likely fix paths: (a) move the entrypoint DB-create probe into the app process so it shares the same `_coldStartConsumed` static; (b) make the sim apply per-process and per-boot (re-arm on every new process); (c) confirm lazy-init really is lazy and the `Database self-initialized` log in container output is not from app boot. |
