...
| CS53-11 | ~~**Investigate: cold-start sim env var not producing visible cold-start in browser today.**~~ Resolved 2026-04-24 — **not a bug.** End-to-end browser validation against a freshly recreated container with `GWN_SIMULATE_COLD_START_MS=45000` produces the expected sequence: 9× `503` + Retry-After across ~50s with 4 distinct progressive-loader messages cycling (`MESSAGE_SETS.leaderboard`), then `200` at t≈54s. The earlier "init at 2s" observation was an earlier visit consuming the one-shot `_coldStartConsumed` slot before logs were inspected. The lazy-init + sim mechanism is working as designed. | ✅ Done — closed as not-a-bug | Captured in `scripts/validate-cold-start-ui.js` which drives a real browser through the cold-start UX: loads `https://localhost`, clicks Leaderboard (a logged-out-accessible page that uses `progressive-loader`), and asserts loader messages cycle + at least one 200 follows. Use this as the manual self-validation hook before requesting reviews on cold-start related changes. |
...
Browser-based validation today requires login because no public endpoint touches the DB. **Out of scope for CS53-10** — track separately if needed (likely as part of CS53-11 fix or a tiny standalone ticket). Operator validation via `container:validate --mode=...` and unit/integration tests cover the regression surface; manual browser validation already requires login (henrik account exists in the seed data).
...
2. **E2E banner check (Playwright) in CS53-10 or split as CS53-10b?** Tentative recommendation: split — CS53-10 stays HTTP-level + unit; CS53-10b adds Playwright after CS53-11 unblocks browser-based validation generally.
3. **Public DB-touching endpoint for logged-out validation:** out of scope here; track in CS53-11 or a fresh ticket.
