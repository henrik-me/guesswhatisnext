...
| CS42-2 | Immediate fix — bump `CACHE_NAME` to `gwn-v3` | ⬜ Pending | Bump `CACHE_NAME` `gwn-v2` → `gwn-v3` in `public/sw.js`. Existing `activate` handler (`sw.js:42-47`) already purges old `gwn-*` caches — keep as-is. Add unit test covering activate-handler purge logic via a mocked `caches` API (no full SW harness). Note: `gwn-v3` on its own does **not** repair already-controlled users on their first post-deploy visit — CS42-2b covers that. |
| CS42-2b | SW upgrade / migration semantics | ⬜ Pending | In `public/js/sw-register.js`, listen for `navigator.serviceWorker.controller` change (`controllerchange` event) and perform a single `window.location.reload()` gated by a "already reloaded once" flag to prevent loops. This migrates `gwn-v2`-controlled users to the new SW within the same session instead of the next navigation. Add a unit test for the "reload once" guard. If a reload-on-upgrade is rejected on UX grounds, the alternative is an explicit banner + "Reload for updates" button — decide in PR review. |
| CS42-3 | ProgressiveLoader 503 auto-retry | ⬜ Pending | Two-part change: (a) **Server:** add `Retry-After` HTTP header alongside the existing JSON `retryAfter` body on `server/app.js:173,177`. This is the canonical retry signal; SW-synthesised offline 503s (`public/sw.js:79-85`) don't have it and are therefore **not** retried. (b) **Client:** at each of the 4 `progressiveLoad` fetchFn call sites (`public/js/app.js:1076, 1236, 2780, 3601`), detect 503 + a present retry signal (header preferred, JSON body fallback) and throw a typed `RetryableError { retryAfterMs }`. In `progressive-loader.js`, split `timers` into **message-escalation timers** (persist across retries) and a **per-attempt request-timeout timer** (cleared every attempt). On `RetryableError`: clear the request-timeout timer only, do not touch message timers, sleep `retryAfterMs` clamped 2000–8000ms, re-run `fetchFn`. Enforce a **35s wall-clock warmup cap** from first attempt start; each attempt's effective timeout is `min(DEFAULTS.timeout, remainingWarmupMs)` so we cannot drift to ~50s. After 35s, fall through to the existing retry-button path. `AbortError` is NOT a `RetryableError`. Unit tests in `progressive-loader.test.js` must cover: (i) 503 with header → retry, (ii) 503 without header/body signal → no retry (offline), (iii) wall-clock cap honored, (iv) message timers persist across retries, (v) `AbortError` still terminates, (vi) ellipsis dots present on loading messages and absent on Retry state, (vii) new 6 s / 20 s message timers fire. **Special case:** `fetchProfile()` (`public/js/app.js:2780-2826`) uses `Promise.allSettled` and tolerates partial failures — update it so that **any** `RetryableError` in the batch promotes the whole load to retryable (do not render partial data while some sub-requests are mid-warmup). **Message content additions** (close the 7 s gap on achievements/community and the 20 s tail on all screens): add `after: 6000` step to `achievements` ("Counting your wins 🏅") and `community` ("Checking for fresh puzzles 🧩"); add `after: 20000` step to all four — leaderboard: "Almost there — servers stretching their legs 🦵"; profile: "Almost there — dusting off your trophy shelf 🏆"; achievements: "Almost there — assembling the trophy wall 🎖️"; community: "Almost there — unpacking the puzzle box 📦". **Animated ellipsis** added to `.progressive-message` template in `setMessage()` — CSS-only (staggered opacity pulse, no `translateY`), `aria-hidden="true"` on dots (live text is already announced via `aria-live="polite"`), colors via `currentColor` to theme correctly in light/dark, respects `prefers-reduced-motion` (dots remain visible but don't animate). Retry state uses its own template and naturally has no dots. |
| CS42-4 | Content-hashed `CACHE_NAME` (drift-proofing) | ⬜ Pending | Build-time solution so the CS42-1 bug cannot recur. `public/sw.js` **stays committed** (no gitignore) so fresh checkouts / `npm start` / tests / Docker all keep working without a build step having run. Approach: tiny Node script (`scripts/build-sw.js`) invoked as `npm run build:sw`, wired into the `prebuild` script and the Dockerfile, that (a) reads a `public/sw.template.js` template, (b) computes a SHA-256 digest over **the template text + the sorted `STATIC_ASSETS` path list + the file contents of each listed asset** (so SW-logic changes or path-list changes also rotate the hash), (c) writes/rewrites `public/sw.js` with `CACHE_NAME = 'gwn-{digest8}'` inlined. Output is deterministic and idempotent — `npm run build:sw && git diff --exit-code public/sw.js` is clean if nothing changed. Add a `check:sw-cache-fresh` test (or extend `check-docs-consistency`) that fails CI if `public/sw.js` is out-of-date. Document the mechanism in `CONTRIBUTING.md` with **links** (no restated values) to `scripts/build-sw.js`, `package.json` scripts, and the Dockerfile step — per "Link, don't restate" (INSTRUCTIONS.md § Documentation Conventions). This is a **code/config PR** (app, script, Dockerfile, package.json, tests) — full validation suite + Copilot review. |
| CS42-5a | E2E — SW upgrade/migration path | ⬜ Pending | New Playwright test (separate spec file from `coldstart-real.spec.mjs` to avoid its SW-blocked fixture). With SW enabled: (1) seed a `gwn-v2` cache containing a distinctive old `app.js` fixture, (2) reload, confirm SW activates new `CACHE_NAME`, purges `gwn-v2`, and — with CS42-2b in place — reloads once into the new shell serving the current `app.js`. Depends on **CS42-2 or CS42-4** merged (either mechanism produces a new CACHE_NAME); also depends on CS42-2b. |
