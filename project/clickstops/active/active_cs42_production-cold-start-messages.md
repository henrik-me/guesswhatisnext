...
3. **CS42-3 timer handling split** — message-escalation timers persist across retries, per-attempt request timeout does not. 35s cap is a wall-clock cap via `min(timeout, remainingWarmupMs)`, so we can't drift to ~50s. `AbortError` explicitly excluded from `RetryableError`.
...
- Azure SQL auto-pause resume takes 10–30s — the ProgressiveLoader's 15s default timeout is too tight. CS42-3 introduces a 35s **wall-clock** warmup window applied only to the retry-signal loop; the normal timeout is unchanged for non-retryable errors.
