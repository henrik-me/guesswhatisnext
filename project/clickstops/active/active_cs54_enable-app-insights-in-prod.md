...
**Recommendation.** **Option C.** Pick this now and ship it as the resolution to Gap 3 in the same PR that resolves Gap 2 (which is recommended as its Option A — Pino → AI). Once Pino logs land in the `traces` table with `err.stack` preserved by Pino's `err` serializer, the marginal value of also calling `recordException` is the AI portal's Failures-blade grouping UI — useful but not worth the cost of introducing an AI-SDK dependency in application code. Option B (global handlers) stays out regardless: the supervisor-conflict trade-off is real and is not the right thing to fold into observability work. Option A becomes the right answer **only** if one of the specific CS54-6/CS54-8 signals below fires.

Note on sequencing: if Gap 2's recommendation is overridden in the future toward its Option C (defer Pino → AI indefinitely), then Gap 3 should be flipped to its Option A — that compensating move is the only escape hatch from "errors are invisible in AI." Capture this if/when the Gap 2 decision is revisited.
...
| 3 — `exceptions` | Option C — rely on Gap 2's Pino → AI pipeline | Folded into Gap 2's CS | No separate CS |
