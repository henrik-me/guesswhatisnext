...
| CS54-3 | Wire `APPLICATIONINSIGHTS_CONNECTION_STRING` into `staging-deploy.yml` via `secretRef:`: deploy template env enumeration (~L464) only. **Do NOT** add the var to the ephemeral smoke-test container env block (~L60–L100). | ✅ Done | wt-1, branch `yoga-gwn-c2/cs54-3-staging-deploy-wiring`. PR link added in commit body. |
