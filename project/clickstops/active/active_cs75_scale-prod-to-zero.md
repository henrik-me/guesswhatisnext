...
|---|------|--------|------------|-------|
| CS75-1 | Update [`.github/workflows/prod-deploy.yml`](../../../.github/workflows/prod-deploy.yml) line ~338 from `--min-replicas 1` → `--min-replicas 0`, keep `--max-replicas 5` unchanged. Single PR. Run local `npm run container:validate` + Copilot review per the standard non-docs PR gate. | ✅ Done | CS73 merged | One-line YAML change. PR body must include `## Container Validation` table per gate. Done in PR [#337](https://github.com/henrik-me/guesswhatisnext/pull/337) (commit `d8e7bc8`; CLI flag was a no-op under `--yaml`, so the actual fix embeds `scale.minReplicas: 0` inside the YAML template — see PR body § Why a YAML embed instead of the CLI flag). |
