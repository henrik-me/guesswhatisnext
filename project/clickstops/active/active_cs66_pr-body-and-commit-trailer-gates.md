...
| CS66-1c | Add unit tests under `tests/check-pr-body.test.js` and `tests/check-commit-trailers.test.js` with fixture PR bodies and fixture git logs. Cover: passing cases, each failure mode, docs-only escape, allowlist behavior. | parallel after 1a + 1b interfaces stabilize |
| CS66-1d | Add `package.json` scripts: `check:pr-body`, `check:commit-trailers`. | parallel |
| CS66-1e | Add `.github/workflows/pr-checks.yml` jobs that run both gates on `pull_request: [opened, synchronize, edited]`. The PR-body check runs from the action context; the commit-trailer check uses `actions/checkout@v4` with `fetch-depth: 0`. | parallel after 1a + 1b exist |
