...
| CS66-2a | Tighten `scripts/check-pr-body.js` to eliminate the false-positive patterns surfaced by the 2026-05-01 audit (40-PR retroactive run): (FP-1) loosen `not applicable (<category>)` regex to allow inline clarification text inside the parens before the close; (FP-2) accept hybrid category markers (`docs+tooling`, `docs+CI`, `tooling+docs`, etc.) by matching all canonical category tokens regardless of separator; (FP-4) accept `N/A` as a synonym for `not applicable`. Keep the table-required rule for Local Review (FP-3 is by design — table over bullets). Improve failure messages to include the canonical templates so authors know the exact wording to use. Add unit tests covering each new accept case + each rejection case. Re-audit confirms zero false positives across the same 40-PR window. | parallel within CS66-2 |
