...
| CS77-2b | Add a side-effect-free `npm run check:hook` script that probes whether the pre-push hook is currently active in this clone: assert `git config core.hooksPath` resolves to `.husky` or `.husky/_` (husky 9 wires the latter; trailing slashes normalized), assert `.husky/pre-push` exists (and the `.husky/_/pre-push` wrapper too when hooksPath is `.husky/_`, since `.husky/_` is gitignored and removable), assert it is executable on POSIX (Windows: existence is sufficient — Git on Windows handles exec-bit equivalence via `core.fileMode`). Exit 0 with `✓ CS77 pre-push hook is active in this clone` or non-zero with `✗ CS77 pre-push hook is NOT active in this clone — run \`npm install\` to activate (see CS77).` Implementation in `scripts/check-hook.js` (~60 lines including the wrapper-existence + git-error-distinction logic; exposed as a reusable `isHookActive(repoRoot)` library function consumed by check-docs-consistency.js's CS77-2c warning). Wire into `package.json` scripts as `check:hook`. | ✅ Done | CS77-1, CS77-2 | PR #328. Probe accepts `.husky` or `.husky/_` (husky 9 sets the latter); distinguishes "git config unset" from "not a git repo / git not on PATH" in the failure reason. |
| CS77-2c | Have `scripts/check-docs-consistency.js` invoke `check-hook` at the top of its run and print a **warning** (not an error — does not affect exit code) **to stderr** if the hook is not active. Suppress the warning entirely when running in any CI environment (any truthy `process.env.CI` value — `true`, `1`, etc.; only `false`/`0`/empty count as not-CI; GitHub Actions sets `CI=true`) AND when `--json` mode is in effect (so machine-readable output is not corrupted — emit to stderr in JSON mode if appropriate, or suppress entirely). The warning text must name CS77 and explain the one-time `npm install` fix. Rationale: an orchestrator who is following today's manual-lint discipline ("run `check:docs:strict` before every direct-to-main push") will get a loud nudge the very next time they lint, converting CS77 rollout from "passive opt-in" to "self-correcting on first lint." | ✅ Done | CS77-1, CS77-2, CS77-2b | PR #328. |
...
| CS77-5 | **Cross-machine / per-checkout activation tracking.** `git pull` does NOT activate husky hooks — Git intentionally never auto-runs versioned hooks on pull, and husky's `core.hooksPath` is set by the `prepare` script which only runs during `npm install`. **Each existing orchestrator checkout listed in [WORKBOARD.md Orchestrators table](../../../WORKBOARD.md) needs to run `npm install` once after CS77 merges to activate the hook locally.** Ack table below — each active orchestrator records "✅ hook active in <checkout-path>" with their agent ID + date. Closure of CS77-5 requires every 🟢 Active orchestrator to have ticked their box; ⚪ Offline orchestrators are tracked but do not block (they will pick up on next return). | 🔄 In Progress | CS77-1 merged (PR #328 merged 2026-05-09T20:01Z, commit `8608674`) | Self-correcting: CS77-2c warns on every `npm run check:docs:strict` run when the hook is missing, so any orchestrator following the manual lint discipline gets a loud nudge automatically. |

#### CS77-5 per-clone ack ledger

Each row recorded by the orchestrator after running `npm install` and verifying via `npm run check:hook` in their clone.

| Agent ID | Clone path | Status | Date acked |
|---|---|---|---|
| yoga-gwn | `C:\src\guesswhatisnext` | ✅ Hook active (`check:hook` returned 0; `core.hooksPath` = `.husky/_`) | 2026-05-09T20:05Z |
| yoga-gwn-c2 | `C:\src\guesswhatisnext_copilot2` | ⏳ pending | — |
| yoga-gwn-c3 | `C:\src\guesswhatisnext_copilot3` | ⏳ pending | — |
| yoga-gwn-c4 | `C:\src\guesswhatisnext_copilot4` | ⚪ Offline (per WORKBOARD; non-blocking) | — |
| yoga-gwn-c5 | `C:\src\guesswhatisnext_copilot5` | ⏳ pending | — |
| omni-gwn | `C:\src\guesswhatisnext` | ⏳ pending | — |
| omni-gwn-c2 | `C:\src\guesswhatisnext_copilot2` | ⚪ Offline (per WORKBOARD; non-blocking) | — |
| omni-gwn-c3 | `C:\src\guesswhatisnext_copilot3` | ⏳ pending | — |
