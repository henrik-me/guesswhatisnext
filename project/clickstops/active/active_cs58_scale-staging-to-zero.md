...
|---|------|--------|------------|-------|
| CS58-1 | Update `staging-deploy.yml` deploy YAML template to `minReplicas: 0` so future deploys don't re-introduce always-on. Validate via `workflow_dispatch` that smoke + E2E still pass with extra cold-start slack. | ✅ Done (PR TBD) | — | One-line YAML change at the `scale:` block in the `deploy-azure-staging` job. |
