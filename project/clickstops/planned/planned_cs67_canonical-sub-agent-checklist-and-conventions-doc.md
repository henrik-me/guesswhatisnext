...
|---------|-------------|-----------|
| CS67-1 | Phase 1 — extract canonical sub-agent checklist. Create `docs/sub-agent-checklist.md` with the verbatim 17-step list. Replace OPERATIONS.md § Sub-Agent Checklist body with a `<!-- include: docs/sub-agent-checklist.md -->` marker + a one-line link. Add `docs:strict` rule `sub-agent-checklist-canonical` (warn-only landing, flipped in CS65-2 follow-up) that asserts the OPERATIONS.md include marker and the canonical file are byte-identical (or, if include resolution lands later, asserts the link target exists). | sequential (Phase 1) |
