...
1. CS77-1 + CS77-2 PR merged (husky installed, pre-push hook in place).
2. CS77-3 docs PR merged.
3. CS77-4 validation evidence captured in the CS77-1+2 PR body (blocking output + `--no-verify` bypass output).
4. CS77-5 cross-machine notice posted (post-merge commit message or WORKBOARD update line).
...
- **`npm install` is required in each checkout** for the hook to activate. `git pull` does NOT activate husky hooks — Git intentionally never auto-runs versioned hooks on pull. CS77-5 mitigates by per-checkout ack-tracking; long-term mitigation is that `npm install` is part of the standard "after pull" reflex anyway.
...
- **Worktrees** (e.g. `git worktree add` for parallel CS work) share `.git/config`'s `core.hooksPath` with the parent repo, so they inherit the hook automatically — no extra activation needed.
...
- **Bypasses remain available** — `git push --no-verify`, `HUSKY=0 git push …`, and `~/.config/husky/init.sh` global disablement. This is by design; CS77 makes silent skip impossible, not deliberate skip impossible. CS77-3 documents that using any bypass requires a documented justification in the same commit/PR.
...
- **Rollback:** remove the `prepare` script + `husky` devDep + `.husky/` directory in a single revert PR. Single command for an in-place uninstall: `npm uninstall husky && rm -rf .husky` plus removing the `prepare` line from `package.json`.
