...
| CS43-7 | Flip consistency check to fail-on-error in `ci.yml` | 🔄 In Progress | Only after CS43-2 through CS43-6 leave the checker passing against `main`. Single-line workflow change. PR description summarizes the gate going live and lists the invariants now enforced. |
