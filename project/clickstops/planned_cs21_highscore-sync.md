...
|----|------|--------|-------|
| CS21-1 | Sync high score from backend on login | ⬜ Pending | After successful login, fetch `/api/scores/me` and update localStorage `gwn_high_score` with the backend's `MAX(score)`. This ensures the displayed high score reflects the actual account data, not stale localStorage. |
...
- `Storage.setHighScore()` in `storage.js` stores a single global value in localStorage key `gwn_high_score`.
- Login handler in `app.js` calls `submitPendingScores()` but does NOT fetch or sync the backend high score.
- `/api/scores/me` returns per-mode `high_score` as `MAX(score)` — data is available but not consumed on login.
- High score is displayed in home screen footer below login controls (`index.html:50`).
