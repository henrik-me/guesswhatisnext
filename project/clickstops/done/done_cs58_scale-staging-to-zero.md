...
**Status:** ✅ Complete (closed by yoga-gwn-c3 on 2026-04-25T19:18Z; cold-wake probe + cost-soak verification both folded into **[CS59](../planned/planned_cs59_staging-cost-soak-verification.md)**)
...
| CS58-5 | **End-to-end functional validation via `staging-deploy.yml workflow_dispatch`.** This is the unaddressed half of CS58-1's acceptance criterion ("Validate via `workflow_dispatch` that smoke + E2E still pass with extra cold-start slack") and the closure step CS58-2 missed. Verify that a fresh `staging-deploy.yml` run, executed against `main` (which contains the CS58-1 YAML change), (a) completes successfully end-to-end, (b) creates an active revision with `minReplicas=0`, and (c) the new revision serves a healthy `/healthz` after wake-from-cold. | ✅ Done — (a) + (b) verified; (c) deferred to CS59 | CS58-1 (merged) | (a) + (b) verified by watcher on yoga-gwn-c2's CS54-6 deploy run [24938066927](https://github.com/henrik-me/guesswhatisnext/actions/runs/24938066927) — workflow completed success; new active revision `gwn-staging--deploy-1777143570` (image `6b368de`) with `minReplicas: 0`, traffic 100%. (c) cold-wake `/healthz` cannot be reliably tested while CS54 is in active staging-validation — concurrent c2 traffic keeps the replica warm, defeating the cold-state assertion. Folded into CS59 pre-flight which runs after a 7-day quiescence window (≥ 2026-05-02) — natural cold state. CS59 plan updated to add the cold-wake probe as an explicit pre-flight step. |
...
4. CS58-5 ✅ workflow_dispatch validation: deploy success + new revision `minReplicas=0` confirmed via real `staging-deploy.yml` run [24938066927](https://github.com/henrik-me/guesswhatisnext/actions/runs/24938066927). Cold-wake `/healthz` proof folded into CS59 pre-flight (cannot test reliably now — concurrent CS54 staging activity keeps the replica warm, defeating cold-state assertion).

CS59 (7-day cost soak + cold-wake probe) appends results back to this file post-closure but does NOT block closure — it's a deferred verification of the cost projection and the cold-state behavior, not an acceptance criterion of CS58 itself.
