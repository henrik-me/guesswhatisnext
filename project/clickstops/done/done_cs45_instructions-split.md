...
| CS45-6 | Update all cross-references in other docs | ✅ Done ([#219](https://github.com/henrik-me/guesswhatisnext/pull/219)) | `CONTEXT.md`, `WORKBOARD.md`, `README.md`, `CONTRIBUTING.md`, `LEARNINGS.md`, every clickstop file (`grep -rn 'INSTRUCTIONS.md'`). Where a reference points at a section that moved, update it to point at the new file + heading anchor. **`.github/copilot-instructions.md` keeps pointing at `INSTRUCTIONS.md` only** (which now serves as the index). Subagents/orchestrators discover the procedure files via the INSTRUCTIONS.md header. The consistency checker (CS43-2 link-resolution rule) must pass. |
| CS45-7 | Update issue #198 with CS45 completion summary | ✅ Done | Final task. Commented on [#198](https://github.com/henrik-me/guesswhatisnext/issues/198) listing what landed (split into INSTRUCTIONS / OPERATIONS / REVIEWS / TRACKING, cross-references updated, walkthrough in PR description) and noting CS45 addresses review.md finding 4. Issue left open as a tracking thread for any remaining product/code findings. |
