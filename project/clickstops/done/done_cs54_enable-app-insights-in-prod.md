...
**Important post-CS54 finding (2026-04-25T23:13Z, captured during the post-close local-container telemetry validation prompted by INSTRUCTIONS.md § 4a):** the original framing below ("mssql instrumentation is intentionally disabled") is **partially incorrect**. The local OTLP collector trace dump (`docker cp <collector>:/data/traces.json`) shows 4× `execSql gwn` spans with `kind=CLIENT` per leaderboard request — i.e. the underlying `tedious` driver IS emitting OTel client spans, and they ARE being exported. They flow to the local file exporter; whether they also flow to Azure Monitor as `dependencies` rows in `gwn-ai-production` is **unverified** — the staging/prod KQL `dependencies | where timestamp > ago(1d)` queries returned empty during CS54-6 verification, but that probe set was not large enough to rule out "spans were exported but Azure Monitor's exporter dropped them on a non-`http`/`express` instrumentation-name filter". A future investigator should: (1) hit `gwn-ai-staging` with a large enough probe set (~20 leaderboard requests) and re-run the `dependencies` query; (2) if still empty, inspect the `instrumentationLibrary.name` attribute on the local `execSql` spans to confirm whether the filter in `server/telemetry.js:23-27` (which uses `instrumentation-http` / `instrumentation-express` names) is silently dropping `tedious` spans on the way to AzMonitorTraceExporter. This finding does not invalidate the recommendation below — it just changes where the work lives (filter widening, not adding a new instrumentation package).

**What's missing.** No `dependencies` rows currently visible in `gwn-ai-staging` / `gwn-ai-production` for outbound MSSQL queries, despite the local OTLP collector showing `execSql` spans being emitted. Either the AzMonitorTraceExporter is dropping them at the export edge, or the probe set during CS54-6 was insufficient to surface them. Today a slow request shows up only as a slow `requests` row with no breakdown of "did the time go in the DB query, in JS, or in the response serialization".

**Why CS54 does not deliver it.** [`server/telemetry.js:12-27`](../../../server/telemetry.js) explicitly filters auto-instrumentations to those whose `instrumentationName` is one of `@opentelemetry/instrumentation-http` + `@opentelemetry/instrumentation-express`. Per the post-CS54 finding above, `tedious` spans appear in the local collector regardless — likely emitted by the driver itself rather than by `getNodeAutoInstrumentations`, so they bypass that filter — but the AzMonitorTraceExporter may still be dropping them. CS54 narrowed scope to the `requests` table to keep the verification surface small; investigating which exporter edge drops the `execSql` spans is the work that belongs in the follow-up.
...
- If `gwn-ai-production` is > 4 GB/month from `requests` alone → flip Gap 1 to Option C (defer); adding `dependencies` cardinality on top would push us into paid tier.
...
| 3 — `exceptions` | If CS60-5 picks Option A and Pino's `err` flows through, this is mostly free (Option C). If CS60-5 stays on the bridge, then add explicit `recordException` / `trackException` in [`server/app.js:575-576`](../../../server/app.js)'s error middleware (Option A); Option B (global handlers) stays out regardless. | Sequenced after CS60-5 | CS60-6 |
