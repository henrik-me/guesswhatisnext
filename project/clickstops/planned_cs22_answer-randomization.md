...
|----|------|--------|-------|
| CS22-1 | Reuse Fisher-Yates shuffle for options | ⬜ Pending | Reuse the existing `shuffle()` function from `public/js/game.js` and apply it to `puzzle.options` before rendering in `renderRound()` (`public/js/app.js`) and multiplayer answer display. Since answer validation already compares by value (not index), no index tracking is needed — just shuffle the display order. |
...
- **Shuffle algorithm:** Reuse the existing Fisher-Yates `shuffle()` function from `public/js/game.js` (already proven for puzzle queue shuffling). Extract or export it for use in option rendering — no need for a second implementation.
...
- `Game.startFreePlay()` shuffles the puzzle queue (which puzzles to show) using an existing `shuffle()` function in `public/js/game.js`, but does NOT shuffle the options within each puzzle.
