...
| CS26-6 | Configure fork PR security | ⬜ Pending | Verify CI workflow (`ci.yml`) does not expose secrets on fork PRs (confirmed safe — uses no repo secrets). Ensure `pull_request` trigger (not `pull_request_target`) is used. Set "Require approval for all outside collaborators" in Actions settings so fork PR workflows need manual approval to run. In contribution guidance, ask fork PR authors to leave "Allow edits from maintainers" unchecked if that is the preferred workflow. |
...
| CS26-11 | Add Quick Reference Checklist to INSTRUCTIONS.md | ⬜ Pending | Add a "Quick Reference Checklist" section at the very top of INSTRUCTIONS.md (before § 1. Architecture Principles) with the 10-15 most critical orchestrator rules. Update the stale instructions guard to say: "After every git pull, re-read the Quick Reference Checklist at the top of INSTRUCTIONS.md regardless of whether the file changed." This improves agent compliance by putting frequently-missed rules in the highest-context-weight position. |
