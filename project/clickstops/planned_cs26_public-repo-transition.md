...
|----|------|--------|-------|
| CS26-1 | Configure branch protection on main | ⬜ Pending | Require PR reviews before merge (at least 1 approval from henrik-me). Require status checks (lint, test, e2e) to pass. Disallow direct pushes to main (exception: WORKBOARD.md pattern may need adjustment — consider using a bot or always-PR approach for public). No force pushes. Require linear history (squash merges only). Dismiss stale reviews on new pushes. Restrict who can push to main. |
...
| CS26-3 | Restrict workflow permissions | ⬜ Pending | Set repository-level Actions permissions to restrict allowed actions (only allow actions from verified creators or specific allowlist). Set default GITHUB_TOKEN permissions to read-only at the repo level. Pin all third-party actions to SHA instead of tags to prevent supply chain attacks. Actions to pin: `docker/login-action@v3`, `docker/build-push-action@v5`, `azure/login@v2`, `azure/cli@v2`, `actions/github-script@v7`. |
